...
| **Cast Strip** *(presence engine 0.6.4)* | Portrait chips for the Cast-card characters **in the scene** — a layered evidence/veto verdict: own dialogue colour (+10, decisive), a physical arrival/position cue in narration near the name (+4), a physical action in the naming sentence (+2), named in the 📍 header (+3); vetoed by a name that only appears inside someone else's speech, an absence/reported-speech sentence (ring, said, would say, remember, about…), a departure, or a location change without fresh evidence. Present at ≥ 4; only decisive evidence earns the 2-message grace. Cue/veto lists editable; `tools/presence-harness.mjs` replays a chat. Previously: — they spoke (dialogue colour) or their name appears next to a presence cue. Silent characters linger dimmed for up to two messages; a departure phrase or a 📍 location change clears them. *(0.5.3)* Only the dialogue colour is strong evidence; a name or kinship alias (cast-card field) needs an arrival/speech cue within ~40 chars and is ignored inside phone/absence sentences ("ring her later"); every add/remove is logged with its evidence *(0.8.3)* 📞 **phone chips**: a member speaking down a line ("over the phone", "hangs up", within ±180 chars of their own lines) shows dimmed with a phone badge — in the scene, not the room; **colour-drift aliasing**: when the model gives a Cast-card character a random dialogue colour, the name in the narration beside it maps that colour to the card for the session instead of spawning a stranger; unknown-speaker name guesses skip number words, weekdays and months |
| **Wardrobe** *(0.8.7)* | Tracks what everyone is wearing from the prose: put-on and take-off verbs, garment phrases with their colour and cut, attributed by possessive or clause ("her green dress", "he takes the suit off"), strangers ignored, memories and hypotheticals ignored, "naked" clears. Rebuilt from a window of recent messages on every message so swipes and edits never leave stale garments. Shows as a 👗 HUD row and in the sprite and chip tooltips, is sent to the model as a `[WARDROBE — …]` line so outfits stop drifting, and `Wardrobe → costume` rules map what she wears to a costume folder (slip → pajamas, ^nothing → nude) ahead of the time rules |
...
| **Cast Mood Bubbles** *(0.2.0)* | Keyword heuristic per NPC chip: a comic thought bubble (😊/😠/😢) and an optional per-mood portrait variant *(0.9.0)* animated `.webp` portraits and mood variants via `npc/animated.json`, still PNG fallback |
| **Drag to position** *(0.8.6)* | Click and drag the character sprite with the mouse to put it anywhere on the screen, and scroll the mouse wheel over it to scale (Shift for fine steps); the X/Y offset sliders follow (now ±80vw / ±60vh), a Reset button puts it back, and the position is saved with your settings |
...
| **Position sprites** *(0.8.9)* | Optional custom labels `riding`, `beneath`, `behind`, `kneeling` for a love-scene costume folder (register them as custom expressions in SillyTavern). Inside an intimate message the last-named position becomes the sprite and sticks until another is named or the scene stops being intimate; the climax still wins its own message; folders without the sprites fall back to the mood |
| **Custom `orgasm` expression** *(0.8.8)* | Add `orgasm` to SillyTavern's custom expressions and give a costume folder an `orgasm` sprite: the mood engine fires it only in an intimate message, on the climax itself (she comes, comes apart, wave after wave, tightens around you, cries out his name), and folders without the sprite fall back to desire. The `[MOOD:]` tag instruction tells the model to use it only for the reply in which she actually climaxes |
| **Mood Engine** *(0.6.0)* | Layered verdict — tag → ST's local go_emotions classifier → weighted lexicon with vetoes — on the character's own dialogue + narration; deterministic rules, 8 s hysteresis, NPC variants from the same pipeline. See *How moods are decided* *(0.8.3)* cue table second pass: every cue is whole-word (no more "encou**rage**"), fear needs a subject, past-tense memories ("I was shaking") and idioms don't count, a head-shake declining an offer is not disapproval, grief needs a bereavement word, and a classifier-only swing from a warm mood into a dark one needs a clear margin — go_emotions reads "I know he'd cry" as sad even when it's tender; *(0.8.4)* a cue inside a remembered moment ("I was so scared on the plane", "I near enough cried") counts at a quarter weight — it is about then, not now; *(0.8.5)* when the model drops the colour tags, plain quoted speech counts as the character's own unless the words before it name another speaker; happy tears (a wet shine plus a smile) read as joy, not sadness |
...
- **A dog on all fours is not a love scene** *(0.9.12)*: `all fours` and `on her knees` sat in the intimacy check with nothing tying them to the character, so *any* subject on all fours made a message intimate and let the pose matcher pick a love-scene sprite from it. Both are now anchored to her, and the pose patterns with them. If you have a dog, a toddler or a gardener in your story, this is the one to update for.
- **A plainly stated feeling counts** *(0.9.11)*: twelve of the commonest feeling words — sad, angry, upset, worried, glad, tired, lonely, surprised, embarrassed and more — had no entry in the mood lexicon, so "She was sad." produced no mood at all. They score low on purpose: the table still prefers what a character *does* over what the prose *says*, so "She was sad but she laughed anyway" is amusement. Each entry is anchored to the character, so "He was angry" is not read as hers.
- **Cast chips appear when they should** *(0.9.10)*: two faults kept a character who was plainly in the room from scoring high enough to show. The action list was present-tense only for a dozen verbs, so `poured`, `shrugged`, `frowned`, `waved`, `pointed`, `sighed`, `picked up`, `shook her head` and `folded her arms` matched nothing — and most roleplay narration is past tense. Separately only the *first* action in a sentence was ever counted, so "She poured the tea and passed him a cup" scored the same as a single nod. One action alone still isn't enough, deliberately: narration describes absent people acting too.
- **Clothes that were never put on** *(0.9.9)*: three sentence shapes made the wardrobe record the opposite of what the prose said. "She did not put her coat on" left her wearing the coat, "She thought about wearing the blue dress" recorded the dress, and `"Put your coat on," she said` recorded a coat nobody touched. Quoted speech is *not* ignored wholesale — a character saying "I'm wearing the green dress" is still real information — only the instruction and reported-speech forms are dropped.
- **"He pulls a shirt on" is a put-on, not a removal** *(0.9.8)*: verbs like `pulls`, `tugs` and `slips` appear in both the put-on and take-off matchers, and the take-off pass ran first, so a garment put on with one of them was claimed as a removal and lost entirely. A neutral verb with no removal word ("off", "out of", "down") beside it is now read as putting on. Unambiguous verbs are untouched, so "she unbuttons her blouse" is still a removal.
- **"She puts her coat on" is tracked** *(0.9.7)*: the wardrobe's put-on matcher required a space where the possessive sits, so the natural English word order — possessive, garment, then the particle — matched nothing at all. `She puts on her coat` worked; `She puts her coat on` was invisible. Fixed, with `tools/wardrobe-harness.mjs` added: it lifts the pure engine straight out of `index.js` and runs a case set in milliseconds instead of a browser round-trip, and asserts the two known limits so they stay visible.
- **Faster on long chats** *(0.9.6)*: the wardrobe is rebuilt from a window of recent messages on every message, edit and swipe, and profiling showed its scanner taking ~13% of the per-message time. Messages that mention no clothing now skip the sentence-by-sentence pass entirely — measured at **39ms → 28ms per message** on a 300-message chat, and more in real stories where most messages mention no clothes. Nothing about what it tracks changes.
- **Drag anything, and "only here when…"** *(0.9.5)*: the cast strip and the scene HUD now drag with the mouse exactly like the character does, each remembering its own place, and the wheel over the strip resizes the chips (Shift for fine steps) — all under the existing *Drag and scale with the mouse* toggle. Cast cards gain an optional **only here when…** regex: set it to a character's world (`phone|farm|the city`) and they can only appear in a message that matches, so someone who lives elsewhere is no longer summoned into the room by being mentioned. Blank leaves behaviour unchanged.
- **Settings that actually save** *(0.9.4)*: **versions 0.8.7 through 0.9.3 had a broken Apply button** — it crashed silently on every click and saved nothing, with a blank status line. That's fixed, and three more things make sure it can't come back: every checkbox now saves the moment you click it (no Apply needed); Apply saves every valid field even when one regex or JSON box has an error (the bad box keeps its inline message and old value, and the status names it); and if the save ever fails again it says so on screen instead of going quiet. Also from this version **Ken Burns drift is off by default** — it was the first thing people tried to turn off and couldn't — it no longer lurches after each generation, it's disabled on phones, and every continuous effect honours your OS *reduce motion* setting. If a setting wouldn't stick for you: update, and it will.
...
- **Animated portraits** *(0.9.0)*: drop an animated `npc/<key>.webp` (and
  ...
  list says one exists and fall back to the PNG if it fails to load; no
  per-card setting. Any breathing-loop tool works (Wan 2.2 i2v via ComfyUI on
  a grey composite, cut out with rembg, ping-ponged — the same recipe as
  `tools/cutout.py` expects).
...
`backgrounds/` ships **164 generic day scenes**, each with a `-night`
