...
(function () {
    ...
    const MoodEngine = (function () {
        ...
        const DEFAULT_LEXICON = [
            ...
            ['nothing\\s+to\\s+give\\s+(?:you\\s+)?back|do\\s?n.?t\\s+deserve\\s+(?:you|this)|best\\s+day\\s+I.?ve\\s+(?:ever\\s+)?had|never\\s+forget\\s+(?:that|this)|you\\s+did\\s+(?:all\\s+)?this\\s+for\\s+me|supposed\\s+to\\s+do\\s+with\\s+a\\s+man', 'gratitude', 3, []],
            ['(?:she|I)\\s+(?:comes?|came)\\b(?!\\s+(?:back|in|out|down|up|over|round|to|home|through|across|forward|closer|away|off|along|round|first|second|out\\s+of|into|from|with\\s+(?:a|the|her|his)\\b|and))|I\'?m\\s+(?:coming|gonna\\s+come|going\\s+to\\s+come)|(?<!it\\s)(?<!they\\s)(?<!one\\s)comes?\\s+(?:hard|undone)|(?:she|he|I|you)\\s+comes?\\s+apart|climax(?:es|ing)?|orgasm|goes\\s+over\\s+the\\s+edge|(?:tips|falls|shatters|breaks)\\s+(?:over|apart)|wave\\s+after\\s+wave|her\\s+release|shudders?\\s+(?:through|toward|into)\\s+(?:it|her\\s+release|release)|(?:clench|tighten)(?:es|ing)?\\s+around\\s+(?:you|him)|cries\\s+out\\s+(?:his\\s+name|your\\s+name)|shakes\\s+through\\s+it|when\\s+it\\s+lets\\s+go|hips\\s+(?:lose\\s+the\\s+rhythm|stutter)|aftershocks?|comes\\s+down\\s+from\\s+it|falls\\s+forward\\s+onto\\s+(?:you|him)|peaks?\\b(?=[^.]{0,40}(?:her|she|body|around))', 'orgasm', 5, ['fear', 'sadness', 'anger', 'annoyance', 'disgust', 'nervousness', 'embarrassment']],
            ...
            ['\\b(?:she|she\'s|she\\s+is|she\\s+was|she\\s+looks|she\\s+sounds|she\\s+feels|I|I\'m|I\\s+am|I\\s+was|I\\s+feel)\\s+(?:\\w+\\s+){0,2}(?:afraid|scared|terrified|frightened|dreading)\\b(?!\\s+of\\s+(?:missing|losing\\s+(?:the|a)\\s+(?:thread|moment)|wasting|forgetting|being\\s+late))', 'fear', 3, []],
        ...
        const INTIMATE_RE = /\b(?:thighs?|hips?|rid(?:es|ing)\s+(?:you|him)|grind|seated|straddl|inside\s+(?:her|me)|climax|orgasm|release|moan|arch(?:es|ing)\s+into|comes?\s+(?:hard|for|apart)|rocks\s+(?:her|against)|bed\s+before\s+coffee|against\s+his\s+mouth|breasts?|nipples?|naked|nude|on\s+her\s+knees\s+(?:in\s+front\s+of|before|between|for)\s+(?:you|him|your|his)|(?:she|her)\s+(?:is\s+|was\s+|gets?\s+|got\s+|goes?\s+|went\s+)?(?:down\s+)?on\s+all\s+fours|from\s+behind|her\s+mouth\s+(?:on|around)|takes?\s+(?:you|him)\s+in(?:to)?\s+her\s+mouth|in\s+her\s+mouth\b|goes\s+down\s+on)\b/i;
    ...
    const PresenceEngine = (function () {
        ...
        const DEFAULT_ABSENCE = 'ring(?:s|ing)?|rang|call(?:s|ed|ing)?(?! (?:out|across|over|up the stairs))|phone[sd]?|phoning|text(?:s|ed|ing)?|messag(?:e|es|ed|ing)|said|says?|would say|tell(?:s|ing)?|told|remember(?:s|ed|ing)?|miss(?:es|ed|ing)? (?:her|him|you|them|it|the|my|his)|wonder(?:s|ed|ing)?|promised?|about(?! to\b)|mention(?:s|ed)?|th(?:ink|ought)s? (?:of|about)|wish(?:es|ed)?|later|tomorrow|yesterday|last (?:night|week|time)|wrote|reckon(?:s|ed)?|thinks?|used to|back (?:home|at)|(?:is|was|\'s) (?:out|off|down|over|up) (?:at|by|with|in|the)\\b|about somewhere|been through|somewhere (?:about|round|around)|the way (?:her|his|my|your|their|our) \\w+(?: \\w+)? (?:does|did|do|would|used to|always)|(?:like|as|how) (?:her|his|my|your|their) (?:mother|father|mama|papa|mum|mom|dad|daddy|grandmother|granny|oma|opa)\\b|(?:mother|father|mama|papa|mum|mom|dad|daddy|grandmother|oma)\'s (?:way|hands|kitchen|voice|words|rule|recipe|habit)'
            // 23 Sep: this table was entirely about REMOTE or reported contact (rang, said, told,
            // remembered) and carried no negation at all, so a flat statement that someone is NOT
            // here did not veto anything. "Beth was not there. Her brother came in instead and took
            // the chair" showed Beth: her name sits within the arrival cue's ±40 char window, so
            // HIS arrival was scored as hers. Stating an absence must be at least as strong as
            // someone else arriving nearby.
            + '|(?:was|is|were|are|had|has)(?:n\'t| not)\\s+(?:there|here|in|home|back|around|with)\\b'
            + '|no sign of|nowhere to be (?:seen|found)|still (?:not|hasn\'t|hadn\'t) (?:back|arrived|come|returned)'
            + '|(?:without|absent|missing|away|gone)\\b(?! (?:with|to (?:say|tell|ask)))';
        const DEFAULT_DEPART = '(?<!the )(?<!gum )(?<!dry )leaves(?! (?:on|of|in|rustl|fall|turn|are|were))|left(?! (?:hand|side|arm|leg|foot|over|behind|it|them|a |the (?:book|bag|plate|cup|notebook|door|keys|tap|light|kettle)))|gone(?! (?:quiet|still|white|pale|red|pink|cold|soft|grey|hard|to sleep|through|off in|wrong))|walk(?:s|ed) out|storm(?:s|ed) (?:out|off)|dr(?:ives?|ove) (?:off|away)|hangs? up|hung up|head(?:s|ed) (?:out|off|home)|goes out|departs?|departed|goodbye|good night|walk(?:s|ed)? off|walk(?:s|ed)? away|head(?:s|ed)? off|wander(?:s|ed)? off|goes off|went off|does not look back|without looking back';
    ...
    const WardrobeEngine = (function () {
        ...
        function scan(text, cast, state, opts) {
            ...
            if (!ANY_CLOTHES_RE.test(src)) return { changes };
            SENT_RE.lastIndex = 0;
            let sm;
            while ((sm = SENT_RE.exec(src)) !== null) {
                ...
                while ((m = ON_RE.exec(sent)) !== null) {
                    // m[4] is the bare "...and into her nightgown" branch: in "she slips out of her
                    // dress and into her nightgown" the only verb belongs to the OFF clause, so the
                    // second garment had no verb of its own and was never put on.
                    const d = m[1] || m[2] || m[3] || m[4]; if (!d) continue;
...
// === POSE ENGINE (pure) BEGIN ===
    ...
    const PoseEngine = (function () {
        const POSES = [
            ...
            ['behind',   /\b(?:from\s+behind|(?:she|her)\s+(?:is\s+|was\s+|gets?\s+|got\s+|goes?\s+|went\s+)?(?:down\s+)?on\s+all\s+fours|on\s+her\s+all\s+fours|(?:her|she)[^.]{0,20}\bhands\s+and\s+knees|bent\s+(?:over|forward)\s+(?:the|a)?|looks?\s+back\s+(?:at\s+(?:you|him)\s+)?over\s+her\s+shoulder|over\s+her\s+shoulder\s+at\s+(?:you|him)|face\s+(?:down|turned)\s+(?:in|into|against)\s+the\s+pillow|presses?\s+her\s+(?:face|cheek)\s+(?:in|into)\s+the\s+(?:pillow|mattress|quilt)|arches?\s+her\s+back\s+(?:and\s+)?(?:pushes|presses)\s+back)\b/i],
    ...
    const BackgroundEngine = (function () {
        ...
        const DEFAULT_NOUNS = [
            ...
            ['kitchen', 'stove|kettle|bench|sink|fridge|oven|toaster|pan\\b|pot\\b|cupboard|microwave|dishwasher|saucepan|chopping board|tea towel|kitchen table'],
            ['dining', 'dining table|place mats?|serving dish'],
            ['living', 'couch|sofa|armchair|telly|television|\\btv\\b|fireplace|coffee table|lounge room|cushions?|mantel(?:piece)?|remote control'],
            ['bedroom', '\\bbed\\b|quilt|pillow|doona|bedside|sheets|mattress|duvet|wardrobe|dresser|chest of drawers|bedspread|headboard'],
            ['bathroom', 'shower|tiles|basin|towel|tap\\b|bathtub|vanity|toilet|bathroom mirror|shower screen|soap'],
